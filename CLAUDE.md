...
- Script'ler `backend/scripts/`: `setup-test-accounts.ps1` (hesap, klinik, danışan kurar, tekrar çalıştırmak güvenli), `test-risk.ps1` (risk uçları, kalıcı kontrol listesi), `test-email-case.ps1` (e-posta büyük/küçük harf ve Türkçe `I` tuzağı), `test-multi-clinic.ps1` (çoklu klinik: yalıtım, ortak takvim, klinik klinik rapor, 53 kontrol), `remove-test-accounts.ps1` (hesapları ve kliniği siler), `TestLib.ps1` (ortak yardımcılar).
